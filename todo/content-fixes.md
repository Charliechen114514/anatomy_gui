...
> 本文件**只收未并入 act 文件的非重复项**——纯文字质量缺陷（错别字 / 病句 / 术语 / 断链），是迁移现有文章时顺手修的机械性修复。
> **不在此重复**：P0 结构 bug / API 高危 / 代码—文章脱节、P1 实质性技术错误、P2 拆分·调序·缺章、P3 配图——这些已在 [`act-1-specimen.md`](act-1-specimen.md) / [`act-3-render.md`](act-3-render.md) / [`act-4-ship.md`](act-4-ship.md) 的「复查待修」节。
> 章节路径以迁移前（旧 `tutorial/native_win32/`、`hands_on_ur_own_gui/handbook/`）为准；迁移到 act 结构后路径相应改变，按内容定位。
...
> 以上为代表项；中严重度病句共约 26 处，迁移时按章通读复核其余。
...
迁移每一篇现有文章时，对照：
1. 本文件（文字质量：错别字 / 病句 / 术语 / 断链）
2. 该章所属 act 文件的「复查待修」节（P0 结构/API/脱节 + P1 技术错误 + P2/P3）
